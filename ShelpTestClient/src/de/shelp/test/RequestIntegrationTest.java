...
public class RequestIntegrationTest {
	...
	@Test
	/*
	 * Testet, ob eine Anfrage für eine Tour erstellt werden kann. Erwartet, das
	 * sich keine Tour selbst zugewiesen werden kann.
	 */
	public void aTestCreateRequest() {
		...
		ReturnCodeResponse createRequest = remote.createRequest(session1
				.getUser().getEmail(), tour.getTour().getId(), "Kommentar",
				session1.getId(), wishes);
	...
	/*
	 * Testet, ob eine Anfrage für eine Tour erstellt werden kann. Erwartet, das
	 * die TargetUser-ID nicht existiert.
	 */
	public void bTestCreateRequest() {
		...
		ReturnCodeResponse createRequest = remote.createRequest(
				"omaErna@gibEsNicht.de", tour.getTour().getId(), "Kommentar",
				session1.getId(), wishes);
		Assert.assertEquals(createRequest.getReturnCode(), ReturnCode.ERROR);
	}
	...
	/*
	 * Testet, ob eine Anfrage für eine Tour erstellt werden kann. Erwartet, das
	 * die Session-ID nicht existiert/ungültig ist.
	 */
	public void cTestCreateRequest() {
		...
		ReturnCodeResponse createRequest = remote.createRequest(session1.getUser().getEmail(), tour.getTour().getId(), "Kommentar",56456456, wishes);
		Assert.assertEquals(createRequest.getReturnCode(), ReturnCode.ERROR);
	}
	...
	public void dTestCreateRequest() {
		...
		ReturnCodeResponse createRequest = remote.createRequest(
				"theresa@sennekamp.de", tour.getTour().getId() + 1,
				"Kommentar", session1.getId(), wishes);
		Assert.assertEquals(createRequest.getReturnCode(), ReturnCode.ERROR);
	}
}
